...
-- ==========================================
-- SCRIPT THÊM DỮ LIỆU ĐÁNH GIÁ (16 TÀI KHOẢN THEO YÊU CẦU)
-- ==========================================
DECLARE @ReviewUsers TABLE (idx INT IDENTITY(1,1), email VARCHAR(255), rating INT, comment NVARCHAR(MAX));
INSERT INTO @ReviewUsers (email, rating, comment) VALUES
('admin_1780813464801@test.com', 5, N'Trải nghiệm tuyệt vời, phòng ốc rất sạch sẽ và tiện nghi, nội thất đẳng cấp. Tôi sẽ quay lại.'),
('admin_1780813553900@test.com', 5, N'Nhân viên phục vụ vô cùng thân thiện và chuyên nghiệp. Một kỳ nghỉ trên cả mong đợi của gia đình tôi.'),
('admin_1780813595619@test.com', 5, N'Khách sạn nằm ở vị trí đắc địa, rất dễ dàng di chuyển tham quan các địa điểm nổi tiếng. Đánh giá 5 sao!'),
('admin_1780813978507@test.com', 5, N'Hồ bơi đẹp và không gian rất thư giãn. Buffet sáng phong phú và hợp khẩu vị. Rất đáng tiền.'),
('admin_1780814386301@test.com', 5, N'Mọi thứ đều hoàn hảo, từ lúc check-in cho đến lúc check-out. Dịch vụ chăm sóc khách hàng đỉnh cao.'),
('admin_1780814604922@test.com', 5, N'Gường cực kỳ êm ái, cho tôi một giấc ngủ ngon sau ngày dài vui chơi. View ngắm cảnh từ phòng tuyệt đẹp.'),
('cust_1780813464801@test.com', 5, N'Giá cả rất phải chăng so với chất lượng dịch vụ mang lại. Tôi hoàn toàn hài lòng với lựa chọn này.'),
('cust_1780813553901@test.com', 5, N'Không gian sảnh lễ tân sang trọng, phòng ốc thoáng mát, có mùi thơm dễ chịu. 10 điểm cho chất lượng.'),
('cust_1780813595619@test.com', 5, N'Cách âm giữa các phòng rất tốt, đảm bảo sự yên tĩnh tuyệt đối. Tiện ích đi kèm cũng rất phong phú.'),
('cust_1780813978507@test.com', 5, N'Thiết kế phòng hiện đại, đầy đủ các trang thiết bị cao cấp. Nhân viên luôn nở nụ cười chào đón.'),
('cust_1780814386301@test.com', 5, N'Kỳ nghỉ thật ý nghĩa và trọn vẹn nhờ sự hỗ trợ nhiệt tình của các bạn nhân viên khách sạn. Xin cảm ơn.'),
('cust_1780814604922@test.com', 4, N'Khách sạn tổng thể rất tốt, phòng sạch sẽ nhưng thang máy đôi khi phải chờ hơi lâu vào giờ cao điểm.'),
('cust1_1780825886147@test.com', 4, N'Dịch vụ ổn định, nhân viên dễ thương. Tuy nhiên, wifi trên phòng thỉnh thoảng hơi chập chờn.'),
('cust2_1780825886147@test.com', 4, N'Bữa sáng ngon nhưng thực đơn các ngày chưa thay đổi nhiều. Chỗ để xe cũng hơi hẹp so với quy mô.'),
('direct_rev_1780826406282@test.com', 4, N'Tiện nghi trong phòng khá đầy đủ, giường êm. Nước nóng trong phòng tắm đôi khi phải xả một lúc mới có.'),
('direct_rev_1780826500792@test.com', 4, N'Khách sạn đẹp, vị trí tốt. Điều hòa ban đêm hơi ồn một chút nhưng không ảnh hưởng nhiều lắm.');

DECLARE @Iter INT = 1;
DECLARE @TotalUsers INT = (SELECT COUNT(*) FROM @ReviewUsers);
DECLARE @CurrEmail VARCHAR(255);
DECLARE @CurrRating INT;
DECLARE @CurrComment NVARCHAR(MAX);
DECLARE @CurrUserId BIGINT;
DECLARE @NewBookingId BIGINT;

WHILE @Iter <= @TotalUsers
BEGIN
    SELECT @CurrEmail = email, @CurrRating = rating, @CurrComment = comment FROM @ReviewUsers WHERE idx = @Iter;
    SELECT @CurrUserId = user_id FROM Users WHERE email = @CurrEmail;
    
    IF @CurrUserId IS NOT NULL
    BEGIN
        -- Tạo một booking giả để thỏa mãn ràng buộc khóa ngoại (booking_id) và unique
        INSERT INTO Bookings (user_id, room_id, check_in_date, check_out_date, num_adults, num_children, room_price_snapshot, total_nights, status, booking_code, created_at, updated_at)
        VALUES (@CurrUserId, 1, '2026-01-01', '2026-01-02', 2, 0, 1500000, 1, 'CHECKED_OUT', 'RVW-' + LEFT(REPLACE(CAST(NEWID() AS VARCHAR(36)), '-', ''), 10) + CAST(@Iter AS VARCHAR), GETDATE(), GETDATE());
        
        SET @NewBookingId = SCOPE_IDENTITY();
        
        -- Thêm đánh giá tương ứng
        INSERT INTO Reviews (booking_id, user_id, room_id, rating_overall, rating_clean, rating_service, rating_location, rating_value, comment, is_approved, created_at)
        VALUES (@NewBookingId, @CurrUserId, 1, @CurrRating, @CurrRating, @CurrRating, @CurrRating, @CurrRating, @CurrComment, 1, GETDATE());
    END
    
    SET @Iter = @Iter + 1;
END

PRINT 'Thêm 16 đánh giá mới thành công!';
GO
